...
def eval(model, dataloader, cmam, missing="audio"):
    model.eval()
    total_pred = []
    total_label = []
    total_miss_type = []
    ...
    with torch.no_grad():
        for _, data in enumerate(dataloader):  # inner loop within one epoch
            ...
            generated = cmam(data["V_feat"].float().to(device))

            if missing == "audio":
                data["A_feat"] = generated
                data["L_feat"] = torch.zeros_like(data["L_feat"]).to(device)
                model.set_input(
                    data
                ...
                data["A_feat"] = torch.zeros_like(data["A_feat"]).to(device)
                model.set_input(
                    data
                )  # unpack data from dataset and apply preprocessing
        ...
        for part_name in [
            ...
            uar_part = recall_score(
                part_label, part_pred, average="macro", zero_division=0
            )
            f1_part = f1_score(part_label, part_pred, average="macro", zero_division=0)
            f1_weighted_part = f1_score(
                part_label, part_pred, average="weighted", zero_division=0
            )
            f1_micro_part = f1_score(
                part_label, part_pred, average="micro", zero_division=0
            )
            recall_macro_part = recall_score(
                part_label, part_pred, average="macro", zero_division=0
            )
            recall_weighted_part = recall_score(
                part_label, part_pred, average="weighted", zero_division=0
            )
            recall_micro_part = recall_score(
                part_label, part_pred, average="micro", zero_division=0
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_pretrained_encoders", action="store_true", default=False)
    parser.add_argument("--epochs", type=int, default=20)
    args = parser.parse_args()
    epochs = args.epochs
    checkpoints_path = args.checkpoints_path

    source_modality = "V_feat"
    target_modality = args.target_modality
    target_net = None

    if target_modality == "L_feat":
        target_net = "net_L"
    elif target_modality == "A_feat":
        target_net = "net_A"
    elif target_modality == "both":
        target_net_V = "net_A"
        target_net_L = "net_L"
    ...
    for cv in range(1, 1 + args.total_cv):
        ...
        trained_video_encoder = LSTMEncoder(
            opt.input_dim_v, opt.embd_size_v, embd_method=opt.embd_method_v
        )
        ...
        if "net_A" in target_net:
            trained_target_encoder = (
                LSTMEncoder(opt.input_dim_a, opt.embd_size_a, opt.embd_method_a)
                .to(device)
                .eval()
            )
            trained_target_encoder.load_state_dict(
                torch.load(
                    os.path.join(model_dir, find_net_in_dir(model_dir, target_net))
                )
            )
        elif "net_L" in target_net:
            trained_target_encoder = (
                TextCNN(opt.input_dim_l, opt.embd_size_l).to(device).eval()
            )
            trained_target_encoder.load_state_dict(
                torch.load(
                    os.path.join(model_dir, find_net_in_dir(model_dir, target_net))
        ...
        cmam = (
            CMAMV1(
                ...
                input_dim=opt.embd_size_v,
                target_dim=(
                    opt.embd_size_a if target_modality == "A_feat" else opt.embd_size_l
                ),
            )
            .to(device)
            .train()
        ...
        ## Create main data loaders for training and validation
        ## Both of these should be set to use the multimodal data set, not the missing one
        ...
        ## Make sure we have the right CV number since the conf file only takes the last one
        opt.cvNo = cv

        ## Create the test data loader which should be set to use the missing data set [AZZ, etc]
        old_dataset_mode = opt.dataset_mode
        opt.dataset_mode = (
            "multimodal_miss"
            if opt.dataset_mode != "cmu_mosei_miss"
            else "cmu_mosei_miss"
        )
        ...
        for epoch in range(epochs):
            ...
            for _, data in enumerate(train_dataloader):
                optimizer.zero_grad()
                audio = data[source_modality].float().to(device)

                with torch.no_grad():
                    target = trained_target_encoder(data[target_modality].to(device))

                generated = cmam(audio)
                loss = criterion(generated, target)

                if target_modality == "A_feat":
                    data["A_feat"] = generated
                    data["L_feat"] = torch.zeros_like(data["L_feat"]).to(device)
                    model.set_input(
                        data
            ...
            if val_loss < best_validation_loss:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(
                        model_dir, f"cmam_{source_modality}_{target_modality}_loss.pt"
                    ),
                )
            ...
            (
                ...
                val_dataloader,
                cmam,
                missing="text" if target_modality == "L_feat" else "audio",
            )
            ...
            if f1 > best_validation_f1:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(
                        model_dir, f"cmam_{source_modality}_{target_modality}_f1.pt"
                    ),
                )
            ...
            if acc > best_validation_acc:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(
                        model_dir, f"cmam_{source_modality}_{target_modality}_acc.pt"
                    ),
                )
            ...
            if uar > best_validation_uar:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(
                        model_dir, f"cmam_{source_modality}_{target_modality}_uar.pt"
                    ),
                )
        ...
        torch.save(
            cmam.state_dict(),
            os.path.join(
                model_dir, f"cmam_{source_modality}_{target_modality}_final.pt"
            ),
        )
        metrics = {
            "accuracy": acc_part,
