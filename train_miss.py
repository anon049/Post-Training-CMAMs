...
def eval(model, val_iter, is_save=False, phase="test"):
    ...
    if is_save:
        ...
        for part_name in ["azz", "zvz", "zzl", "avz", "azl", "zvl"]:
            ...
            np.save(
                os.path.join(save_dir, "{}_{}_pred.npy".format(phase, part_name)),
                part_pred,
            )
            np.save(
                os.path.join(save_dir, "{}_{}_label.npy".format(phase, part_name)),
                part_label,
            )
            if phase == "test":
                recorder_lookup[part_name].write_result_to_tsv(
                    {"acc": acc_part, "uar": uar_part, "f1": f1_part}, cvNo=opt.cvNo
                )
...
def clean_chekpoints(expr_name, store_epoch):
    ...
    for checkpoint in os.listdir(root):
        if not checkpoint.startswith(str(store_epoch) + "_") and checkpoint.endswith(
            "pth"
        ):
...
if __name__ == "__main__":
    ...
    recorder_lookup = {  # init result recoreder
        "total": ResultRecorder(
            os.path.join(result_dir, "result_total.tsv"), total_cv=total_cv
        ),
        "azz": ResultRecorder(
            os.path.join(result_dir, "result_azz.tsv"), total_cv=total_cv
        ),
        "zvz": ResultRecorder(
            os.path.join(result_dir, "result_zvz.tsv"), total_cv=total_cv
        ),
        "zzl": ResultRecorder(
            os.path.join(result_dir, "result_zzl.tsv"), total_cv=total_cv
        ),
        "avz": ResultRecorder(
            os.path.join(result_dir, "result_avz.tsv"), total_cv=total_cv
        ),
        "azl": ResultRecorder(
            os.path.join(result_dir, "result_azl.tsv"), total_cv=total_cv
        ),
        "zvl": ResultRecorder(
            os.path.join(result_dir, "result_zvl.tsv"), total_cv=total_cv
        ),
    }
    ...
    for epoch in range(
        ...
        logger.info(
            "Val result of epoch %d / %d acc %.4f uar %.4f f1 %.4f"
        ...
        if opt.has_test and opt.verbose:
            ...
            logger.info(
                "Tst result of epoch %d acc %.4f uar %.4f f1 %.4f"
                % (epoch, acc, uar, f1)
            )
            logger.info("\n{}".format(cm))
        ...
        if opt.corpus_name == "IEMOCAP":
            ...
            raise ValueError(
                f"corpus name must be IEMOCAP or MSP, but got {opt.corpus_name}"
            )

    logger.info(
        "Best eval epoch %d found with %s %f"
        % (best_eval_epoch, select_metric, best_metric)
    )
    # test
    if opt.has_test:
