...
def eval(model, dataloader, cmam, missing="video"):
    ...
    with torch.no_grad():
        for _, data in enumerate(dataloader):  # inner loop within one epoch
            ...
            generated = cmam(
                data["A_feat"].float().to(device), data["L_feat"].float().to(device)
            )
    ...
    for cv in range(1, 1 + args.total_cv):
        ...
        for epoch in range(epochs):
            ...
            for _, data in enumerate(train_dataloader):
                ...
                with torch.no_grad():
                    target = trained_target_encoder(video)

                generated = cmam(audio, text)
            ...
            if val_loss < best_validation_loss:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(model_dir, "cmam_AT_V_loss.pt"),
                )
            ...
            if f1 > best_validation_f1:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(model_dir, "cmam_cmam_AT_V_f1.pt"),
                )
            ...
            if acc > best_validation_acc:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(model_dir, "cmam_cmam_AT_V_acc.pt"),
                )
            ...
            if uar > best_validation_uar:
                ...
                torch.save(
                    cmam.state_dict(),
                    os.path.join(model_dir, "cmam_AT_V_uar.pt"),
                )
        ...
        torch.save(
            cmam.state_dict(),
            os.path.join(model_dir, "cmam_AT_V_final.pt"),
        )
        metrics = {
            "accuracy": acc_part,
            "uar": uar_part,
