...
class MultimodalMissDataset(BaseDataset):
    ...
    def modify_commandline_options(parser, isTrain=None):
        ...
        parser.add_argument(
            "--output_dim", type=int, help="how many label types in this dataset"
        )
        parser.add_argument(
            "--norm_method",
            type=str,
        ...
        parser.add_argument(
            "--corpus_name", type=str, default="IEMOCAP", help="which dataset to use"
        )
        return parser
    ...
    def __init__(self, opt, set_name):
        ...
        if set_name != "trn":  # val && tst
            ...
            self.miss_type = ["azz", "zvz", "zzl", "avz", "azl", "zvl", "nnn"] * len(
                self.label
            )
        else:  # trn
            self.missing_index = [
    ...
    def __getitem__(self, index):
        if self.set_name != "trn":
            feat_idx = index // 7  # totally 6 missing types
