...
class MMINModel(BaseModel):
    ...
    def n_params(self):
        return (
            sum(p.numel() for p in self.netA.parameters() if p.requires_grad)
            + sum(p.numel() for p in self.netV.parameters() if p.requires_grad)
            + sum(p.numel() for p in self.netL.parameters() if p.requires_grad)
            + sum(p.numel() for p in self.netAE.parameters() if p.requires_grad)
            + sum(p.numel() for p in self.netC.parameters() if p.requires_grad)
        )
    ...
    def forward(self, **kwargs):
        ...
        if self.isTrain:
            with torch.no_grad():
                ...
                self.T_embds = torch.cat(
                    [self.T_embd_A, self.T_embd_L, self.T_embd_V], dim=-1
                )

    def backward(self):
        """Calculate the loss for back propagation"""
        self.loss_CE = self.ce_weight * self.criterion_ce(self.logits, self.label)
        self.loss_mse = self.mse_weight * self.criterion_mse(
            self.T_embds, self.recon_fusion
        )
        self.loss_cycle = self.cycle_weight * self.criterion_mse(
            self.feat_fusion_miss.detach(), self.recon_cycle
        )
