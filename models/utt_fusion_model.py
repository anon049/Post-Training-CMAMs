...
class UttFusionModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument(
            "--input_dim_v", type=int, default=384, help="visual input dim"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--modality", type=str, help="which modality to use for model"
        )
        return parser
    ...
    def __init__(self, opt):
        ...
        cls_input_size = (
            opt.embd_size_a * int("A" in self.modality)
    ...
    def set_input(self, input):
        ...
        if "missing_index" in input:
            ...
            self.lexical = self.lexical * self.L_miss_index

    def forward(self, v_in=None, l_in=None, a_in=None, **kwargs):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        final_embd = []
        if "A" in self.modality and a_in is None:
            self.feat_A = self.netA(self.acoustic)
            final_embd.append(self.feat_A)

        if a_in is not None:
            final_embd.append(a_in)

        if "L" in self.modality and l_in is None:
            self.feat_L = self.netL(self.lexical)
            final_embd.append(self.feat_L)

        if l_in is not None:
            final_embd.append(l_in)

        if "V" in self.modality and v_in is None:
            self.feat_V = self.netV(self.visual)
            final_embd.append(self.feat_V)

        if v_in is not None:
            final_embd.append(v_in)
    ...
    def backward(self):
        ...
        for model in self.model_names:
            torch.nn.utils.clip_grad_norm_(
                getattr(self, "net" + model).parameters(), 0.5
            )
