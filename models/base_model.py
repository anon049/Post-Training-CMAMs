...
class BaseModel(ABC):
    ...
    def setup(self, opt):
        ...
        if self.isTrain:
            self.schedulers = [
                tools.get_scheduler(optimizer, opt) for optimizer in self.optimizers
            ]
    ...
    def test(self, v_in=None, l_in=None, a_in=None):
        ...
        """
        with torch.no_grad():
            self.forward(v_in=v_in, l_in=l_in, a_in=a_in)
    ...
    def load_networks_cv(self, folder_path):
        ...
        """
        checkpoints = list(
            filter(lambda x: x.endswith(".pth"), os.listdir(folder_path))
        )
        for name in self.model_names:
            if isinstance(name, str):
                load_filename = list(
                    filter(
                        lambda x: x.split(".")[0].endswith("net_" + name), checkpoints
                    )
                )
                assert len(load_filename) == 1, "In folder: {}, Exists file {}".format(
                    folder_path, load_filename
                )
