...
def eval(model, dataloader, cmam, missing="audio"):
    ...
    with torch.no_grad():
        for _, data in enumerate(dataloader):  # inner loop within one epoch
            ...
            generated = cmam(data["L_feat"].float().to(device))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_pretrained_encoders", action="store_true", default=False
    )  #
    ...
    ## CMAM Stuff first (as much as possible)
    source_modality = "L_feat"
    ...
    for cv in range(1, 1 + args.total_cv):
        ...
        cmam = (
            CMAMV1(
                ...
                opt.embd_size_l,
                opt.embd_size_a if target_modality == "A_feat" else opt.embd_size_v,
            )
            .to(device)
            .train()
        )
        ...
        for epoch in range(epochs):
            ...
            (
                ...
                val_dataloader,
                cmam,
                missing="video" if target_modality == "V_feat" else "audio",
            )
