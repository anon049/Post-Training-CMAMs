...
def eval(model, dataloader, cmam, missing="video"):
    ...
    with torch.no_grad():
        for _, data in enumerate(dataloader):  # inner loop within one epoch
            ...
            generated = cmam(data["A_feat"].float().to(device))

            if missing == "video":
                data["V_feat"] = generated
                data["L_feat"] = torch.zeros_like(data["L_feat"]).to(device)
                model.set_input(
                    data
                )  # unpack data from dataset and apply preprocessing
                model.test(v_in=generated)
            elif missing == "text":
                data["L_feat"] = generated
                data["V_feat"] = torch.zeros_like(data["V_feat"]).to(device)
                model.set_input(
                    data
                )  # unpack data from dataset and apply preprocessing
...
if __name__ == "__main__":
    ...
    source_modality = "A_feat"
    target_modality = args.target_modality
    target_net = None

    if target_modality == "L_feat":
        target_net = "net_L"
    elif target_modality == "V_feat":
        target_net = "net_V"
    ...
    for cv in range(1, 1 + args.total_cv):
        ...
        opt = OptConfig()
        opt.load(config)
        trained_audio_encoder = LSTMEncoder(
            opt.input_dim_a, opt.embd_size_a, embd_method=opt.embd_method_a
        )

        model_dir = os.path.join(checkpoints_path, str(cv))
        if args.use_pretrained_encoders:
            trained_audio_encoder.load_state_dict(
                torch.load(os.path.join(model_dir, find_net_in_dir(model_dir, "net_A")))
            )
        trained_audio_encoder = trained_audio_encoder.to(device)

        if "net_V" in target_net:
            trained_target_encoder = (
                LSTMEncoder(opt.input_dim_v, opt.embd_size_v, opt.embd_method_v)
                .to(device)
                .eval()
            )
        ...
        cmam = (
            CMAMV1(
                trained_audio_encoder,
                input_dim=opt.embd_size_a,
                target_dim=(
                    opt.embd_size_v if target_modality == "V_feat" else opt.embd_size_l
                ),
            )
            .to(device)
            .train()
        ...
        for epoch in range(epochs):
            ...
            (
                ...
                val_dataloader,
                cmam,
                missing="text" if target_modality == "L_feat" else "video",
            )
            ...
            if f1 > best_validation_f1:
                ...
                torch.save(
                    ...
                    os.path.join(
                        model_dir,
                        f"{'NE' if not args.use_pretrained_encoders else ''}cmam_{source_modality}_{target_modality}_f1.pt",
                    ),
                )
            ...
            if acc > best_validation_acc:
                ...
                torch.save(
                    ...
                    os.path.join(
                        model_dir,
                        f"{'NE' if not args.use_pretrained_encoders else ''}cmam_{source_modality}_{target_modality}_acc.pt",
                    ),
                )
            ...
            if uar > best_validation_uar:
                ...
                torch.save(
                    ...
                    os.path.join(
                        model_dir,
                        f"{'NE' if not args.use_pretrained_encoders else ''}cmam_{source_modality}_{target_modality}_uar.pt",
                    ),
                )
        ...
        torch.save(
            ...
            os.path.join(
                model_dir,
                f"{'NE' if not args.use_pretrained_encoders else ''}cmam_{source_modality}_{target_modality}_final.pt",
            ),
        )
        metrics = {
            "accuracy": acc_part,
